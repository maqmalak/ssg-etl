...
def save_with_update_insert(df, table_name, jdbc_url, jdbc_properties):
    ...
    try:
        ...
        try:
            from sparkFiles.upsert_to_pg import upsert_data_via_spark
            # Create a Spark session for the upsert operation
            from pyspark.sql import SparkSession
            spark = SparkSession.builder.getOrCreate()
            
            # Perform upsert operation
            success = upsert_data_via_spark(spark, df, table_name, ['id'], {
                'jdbc_url': jdbc_url,
            ...
            print(f"Could not import upsert function, falling back to overwrite mode: {e}")
