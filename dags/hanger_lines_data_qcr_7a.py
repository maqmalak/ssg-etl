...
from dags.hanger_lines_qcr_7a import (
    get_source_postgres_engine,
    get_target_postgres_engine,
...
@dag(
    dag_id="hanger_lines_data_qcr_7a",
    default_args=default_args,
    # schedule=timedelta(minutes=5),
    schedule="*/10 8-23,0-1 * * 1-6",  # ✅ Every 9 min, 8AM–2AM, Mon–Sat
...
def hanger_lines_data_qcr():
    ...
    def check_source_connection(connection_id: str, ti=None) -> dict:
        step = "source"
        try:
            with get_source_postgres_engine().connect() as conn:
    ...
    def check_target_connection(connection_id: str, ti=None) -> dict:
        step = "target"
        try:
            with get_target_postgres_engine().connect() as conn:
                if conn.execute(text("SELECT 1")).fetchone():
        ...
        try:
            ...
            try:
                with engine.connect() as conn:
                    # Use proper timestamp comparison for incremental processing
                    if last_extract_dt:
                        count = conn.execute(
                            text("SELECT COUNT(*) FROM pmr_quality_tracking_master WHERE pqtm_date_back > :last_dt"),
                            {"last_dt": last_extract_dt}
                        ).scalar()
                    else:
                        # For initial load, count recent records (last 30 days)
                        count = conn.execute(
                            text("SELECT COUNT(*) FROM pmr_quality_tracking_master WHERE pqtm_date_back >= CURRENT_DATE - INTERVAL '30 days'")
                        ).scalar()
                    if count == 0:
    ...
    for conn_id in SOURCE_HANGER_LANE:
        with TaskGroup(group_id=f"line_{conn_id}", tooltip=f"Pipeline for {conn_id}") as tg:
            ...
            tgt = check_target_connection.override(task_id=f"target_check_pg-ssg")("pg-ssg")
