...
def validate_aggregated_tables():
    ...
    try:
        ...
        oc_total = oc_stats[4] if oc_stats[4] is not None else 0
        shift_total = shift_stats[4] if shift_stats[4] is not None else 0
        employee_total = employee_stats[4] if employee_stats[4] is not None else 0
        
        if oc_total == shift_total == employee_total:
            logger.info("✅ Quantity totals match across all aggregated tables")
        else:
            logger.warning("⚠ Quantity totals do not match across tables")
            logger.warning(f"  odp_date_oc total: {oc_total}")
            logger.warning(f"  odp_date_shift total: {shift_total}")
            logger.warning(f"  odp_date_employee total: {employee_total}")
...
def validate_data_quality():
    ...
    try:
        ...
        for table, key_columns in duplicate_checks:
            key_columns_quoted = [f'"{col}"' for col in key_columns]
            key_columns_str = ", ".join(key_columns_quoted)
            cursor.execute(f"""
