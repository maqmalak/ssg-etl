...
#     # as the Spark (PySpark) script is located in  ${AIRFLOW_PROJ_DIR:-.}/sparkFiles:/opt/airflow/sparkFiles/sparkProcess.py it is executed using a BashOperator
# @retry_on_exception()
# def transform_extract_data(connection_id: str) -> str:
#     """
#         Transform the extracted data using PySpark.
#         This task executes the sparkProcess.py script which:
#         1. Reads data from the hanger_lane_data table
#         2. Aggregates sum(ODPD_Quantity) grouped by OPD_Date and OC_Description
#         3. Saves the result to the opd_oc_date table
#     """
#     logger.info(f"[{connection_id}] Starting data load")
        
#     # Path to the Spark script in the container
#     spark_script_path = "/opt/airflow/sparkFiles/sparkProcess.py"
        
#     # Check if the script exists
#     if not os.path.exists(spark_script_path):
#         logger.error(f"Spark script not found at {spark_script_path}")
#         raise FileNotFoundError(f"Spark script not found at {spark_script_path}")
        
#     # Execute the Spark script
#     try:
#         # Using spark-submit to run the PySpark script
#         # result = subprocess.run(
#         #     ["spark-submit", spark_script_path],
#         #     capture_output=True,
#         #     text=True,
#         #     check=True
#         # )
#         logger.info("Spark transformation completed successfully")
#         # logger.info(f"Spark script output: {result.stdout}")
#         return "Transformation task executed successfully"
#     except subprocess.CalledProcessError as e:
#         logger.error(f"Spark transformation failed with error: {e}")
#         # logger.error(f"Spark script stderr: {e.stderr}")
#         raise
#     except Exception as e:
#         logger.error(f"Unexpected error during Spark transformation: {e}")
#     raise
...
#     @task
#     def transform(connection_id: str) -> str:
#         """
#         transform extracted data and save to PostgreSQL.
...
#             str: Status message
#         """
#         return transform_extract_data(connection_id)
