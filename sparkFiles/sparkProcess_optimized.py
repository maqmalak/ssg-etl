...
def create_spark_session():
    ...
    try:
        ...
        if jdbc_driver_path and os.path.exists(jdbc_driver_path):
            ...
            # Create Spark session with explicit JDBC driver configuration and optimized settings
...
def transform_data(spark, df=None):
    """Transform data to create aggregated tables with optimized memory usage"""
    try:
        print("Starting data transformation...")
        
        # If DataFrame is not provided, load data from database
        if df is None:
            print("No DataFrame provided, loading data from database...")
            ...
            print("Using provided DataFrame for transformation")
            print(f"Provided DataFrame row count: {df.count()}")
        ...
        # Cache the DataFrame for better performance since we'll use it multiple times
        df.cache()
        ...
        df.unpersist()
        print("DataFrame uncached to free memory")
        ...
        try:
            # Don't stop the Spark session here as it might be used by other processes
            # spark.stop()
...
def save_with_update_insert(df, table_name, jdbc_url, jdbc_properties):
    ...
    try:
        ...
        df = df.coalesce(10)  # Reduce number of partitions for better performance
