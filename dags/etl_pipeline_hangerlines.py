...
#     """
#     connection = BaseHook.get_connection("pg_ssg")
#     # uri="postgresql://postgres:postgres@172.16.7.6:5432/ssg?sslmode=disable"
#     uri = f"postgresql://{connection.login}:{connection.password}@{connection.host}:{connection.port}/{connection.schema}?sslmode=disable"
...
#             logger.info(f"Min created_at from source: {result}")
#             return result
#     except Exception as e:
#         logger.error(f"Error fetching min created_at from source: {e}")
#         raise
...
#         if not transaction.get("created_at"):
#             logger.warning("Skipping record with missing created_at")
#             continue
...
# @retry_on_exception()
# def save_to_postgres(transactions: List[Dict[str, Any]], connection_id: str, **context) -> str:
#     """
...
#         transactions (List[Dict[str, Any]]): List of transaction records
#         connection_id (str): Source connection identifier
#         context (dict): Airflow context containing DAG run information
...
#     # Get the DAG run ID from context to use as processlogid
#     dag_run = context.get('dag_run')
#     processlogid = dag_run.run_id if dag_run else str(uuid.uuid4())
#     logger.info(f"[{connection_id}] Using processlogid: {processlogid}")
    
#     # Get last extract datetime from transactions
#     last_extract_dt = (
#         max(tx["created_at"] for tx in transactions if tx.get("created_at"))
...
#         # Log successful completion with the DAG run ID as processlogid
#         insert_etl_log(
#             processlogid,
#             connection_id,
#             saved_count,
...
#         # Log failure with the DAG run ID as processlogid
#         insert_etl_log(
#             processlogid,
#             connection_id,
#             saved_count,
...
#     "start_date": datetime(2025, 8, 14, 19, 0, tzinfo=PKT),
#     "retries": 1,
...
# @dag(
#     dag_id="etl_pipeline_hangerlines",
#     default_args=default_args,
#     schedule="*/30 * * * *",  # Every 10 minutes
#     tags=["ssg", "hanger_lines", "etl"],
#     catchup=False,
#     max_active_runs=1,
#     description="ETL pipeline for Hanger DB data from MSSQL to PostgreSQL",
# )
# def dynamic_hanger_db_etl():
#     """
#     Dynamic ETL DAG for Hanger DB data.
...
#             str: Status message
#         """
#         # The context will be automatically passed by Airflow to the underlying function
